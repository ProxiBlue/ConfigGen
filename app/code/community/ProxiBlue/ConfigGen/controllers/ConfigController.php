...
class ProxiBlue_ConfigGen_ConfigController extends Mage_Adminhtml_Controller_Action {
    ...
    public function configAction() {
        ...
        $block = $this->getLayout()->createBlock('configgen/adminhtml_configform', 'configgen.adminhtml.configform.grid');
        $this->_initAction()
                ->_addBreadcrumb(Mage::helper('adminhtml')->__('Tools'), Mage::helper('adminhtml')->__('ProxiBlue Config Generator'))
                ->_addContent($block)
                ->renderLayout();
    }
    ...
    public function massConfigGenAction() {
        ...
        try {
            ...
            $io->streamUnlock();
            $io->streamClose();
            $this->_prepareDownloadResponse('proxiblue_generated_config.php', array(
                'type' => 'filename',
                'value' => $file,
